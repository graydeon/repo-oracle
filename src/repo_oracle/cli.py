...
def _build_oss_report(scan: dict) -> dict:
    ...
    issues_link = f"https://github.com/{scan['git_remote'].split('github.com/')[-1].replace('.git','')}/issues" if 'github.com' in (scan.get('git_remote') or '') else ""

    # Executive summary
    activity_label = "Active" if scan["has_git"] and scan["commit_count"] > 10 else "Dormant"
    report["executive_summary"] = (
    ...
    report["purpose"] = new_finding(
        ...
        ["scan:readme", "scan:languages"] + ([f"file:{f}" for f in scan["documentation_files"][:2]] if scan["documentation_files"] else []),
    ...
    report["vital_signs"] = {
        ...
        "build_system": ", ".join(scan["build_systems"]) if scan["build_systems"] else "None detected",
    }

    # OSS-specific dimensions
    oss_dims = {
        "purpose": ("HIGH", f"OSS project: {', '.join(top_langs)}. {'Active community' if scan['commit_count'] > 50 else 'Small/early project'}.", "scan:languages", f"{scan['total_files']} files, {scan['commit_count']} commits"),
        "languages_and_frameworks": ("HIGH", f"Primary: {', '.join(top_langs)}. {len(scan['languages'])} languages total.", "scan:languages", "Check language familiarity before contributing."),
        "build_and_test_system": ("HIGH" if has_ci else "MEDIUM", f"{'CI/CD configured' if has_ci else 'No CI detected'}. {'Tests: ' + ', '.join(scan['test_frameworks']) if has_tests else 'No test framework detected'}.", "scan:build", "Build system determines local dev setup complexity."),
        "entrypoints": ("MEDIUM", f"Entrypoints: {', '.join(scan['entrypoints'][:5]) if scan['entrypoints'] else 'Unclear'}.", "scan:entrypoints", "Entrypoints guide first-time explorers to the right starting point."),
        "architecture": ("MEDIUM", f"Structure: {', '.join(scan['top_level_dirs']) if scan['top_level_dirs'] else 'Flat'}.", "scan:top_level_dirs", "Clear architecture helps contributors find their way."),
        "dependency_graph": ("MEDIUM", f"Dependency files: {', '.join(scan['dependency_files']) if scan['dependency_files'] else 'None detected'}.", "scan:dependency_files", "Understand what the project depends on before integrating."),
        "runtime_assumptions": ("MEDIUM", f"Platform: Python {'3.11+' if any('.py' in f for f in scan.get('all_files', [])) else 'unknown'}.", "scan:languages", "Verify your environment matches before contributing."),
        "stale_dependencies": ("MEDIUM", "Check dependency freshness before integrating. Run pip-audit or npm audit.", "scan:dependency_files", "Stale dependencies increase security risk for downstream consumers."),
        "git_history": ("HIGH" if scan['has_git'] else "LOW", f"{scan['commit_count']} commits, last: {scan['last_commit_date'][:10]}. {'Active development.' if scan['commit_count'] > 20 else 'Early stage.'}", "scan:git", "Frequent commits signal active maintenance."),
        "open_todos": ("LOW", "Scan for TODO/FIXME markers to find easy first-contribution opportunities.", "", "TODOs often make great starter issues for new contributors."),
        "documentation_quality": ("HIGH" if has_contrib else "MEDIUM", f"{'CONTRIBUTING guide found' if has_contrib else 'No CONTRIBUTING guide'}. {'LICENSE: ' + [f for f in scan['documentation_files'] if 'LICENSE' in f.upper()][0] if has_license else 'No LICENSE file — project may not be open-source.'}.", "scan:documentation_files", "Documentation quality directly impacts contribution velocity."),
        "test_coverage": ("HIGH" if has_tests else "LOW", f"{'Tests present: ' + ', '.join(scan['test_frameworks']) if has_tests else 'No tests detected — risky to contribute without tests.'}.", "scan:tests", "Good test coverage protects your contributions from regression."),
        "security_surface": ("MEDIUM", f"{'Secret files detected: ' + str(len(scan['secret_files_detected'])) if scan['secret_files_detected'] else 'No secret files detected.'}.", "scan:secrets", "Before contributing, ensure you understand the security posture."),
        "deployment_clues": ("LOW", f"{'CI/CD: ' + ', '.join(scan['ci_cd_files'][:3]) if has_ci else 'No CI/CD'}. Docker: {'Yes' if scan['docker_files'] else 'No'}.", "scan:docker", "CI/CD status tells you if contributions will pass checks."),
        "activity_signal": ("HIGH", f"{'ACTIVE — frequent commits, CI active' if scan['commit_count'] > 20 and has_ci else 'DORMANT — slow development, may accept contributions slowly'}.", "scan:git", ""),
        "value_assessment": ("HIGH", f"{'Good candidate for contribution' if has_contrib and has_ci and has_tests else 'Needs OSS infrastructure before contributing'}. {'Has license' if has_license else '⚠ No license — project may not be open-source!'}.", "scan:documentation_files", ""),
        "next_action_recommendation": ("HIGH", _oss_next_action(has_contrib, has_ci, has_tests, has_license, scan), "scan:documentation_files", ""),
    }

    for i, dim in enumerate(ALL_DIMENSIONS):
        conf, summary, evidence, details = oss_dims[dim]
        report["dimensions"][i] = new_finding(dim, conf, summary, [evidence] if evidence else [], details)

    # OSS recommendations
    recs = []
    if not has_license:
        recs.append(new_recommendation("oss-1", "DOCUMENT", "URGENT",
    ...
    if not has_ci:
        recs.append(new_recommendation("oss-4", "PRODUCTIZE", "MEDIUM",
    ...
    recs.append(new_recommendation("oss-5", "DOCUMENT", "MEDIUM",
    ...
    recs.append(new_recommendation("oss-6", "DEPENDENCY-UPDATE", "LOW",
    ...
    if has_contrib and has_tests:
        ...
        actions.append("3. Add tests for the area you want to change before refactoring")
    actions.append(f"4. {'Integrate via pip install' if 'library' in scan['repo_name'].lower() or 'sdk' in scan['repo_name'].lower() else 'Use as a dependency in your project'}")
    report["next_actions"] = actions

    # Evidence registry
    for eid in ["scan:languages", "scan:build", "scan:tests", "scan:git", "scan:docs",
                "scan:entrypoints", "scan:top_level_dirs", "scan:dependency_files",
                "scan:documentation_files", "scan:ci_cd", "scan:docker", "scan:secrets",
                "scan:files", "scan:readme"]:
        if eid == "scan:readme":
            report["evidence_registry"][eid] = new_evidence(eid, "scan", "README analyzed for project purpose and setup instructions")
        elif eid == "scan:docs":
            report["evidence_registry"][eid] = new_evidence(eid, "scan", f"Documentation files: {scan.get('documentation_files', [])}")

    report["security_notes"] = [
        f"{len(scan['secret_files_detected'])} secret files detected (contents not inspected)." if scan['secret_files_detected'] else "No secret files detected.",
...
def _oss_next_action(has_contrib: bool, has_ci: bool, has_tests: bool, has_license: bool, scan: dict) -> str:
    if not has_license:
...
def investigate(
    ...
    oss: bool = typer.Option(False, "--oss", help="Reframe for open-source contribution or utilization"),
) -> None:
    ...
    try:
        ...
        if not quiet:
            with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"),
                ...
                if tier == "deep":
                    report = _build_oss_report(scan_result) if oss else _build_quick_report(scan_result)
                else:
