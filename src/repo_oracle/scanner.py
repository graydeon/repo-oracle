...
def scan_repo(repo_path: str) -> dict:
    ...
    ci_cd = [
        ...
        for f in file_data["all_files"]
        if any(
            f.startswith(os.path.join(repo_path, c))
            or os.path.basename(f) in CI_CD_FILES
            for c in CI_CD_FILES
        )
    ]
    docker = [f for f in file_data["all_files"] if os.path.basename(f) in DOCKER_FILES]
    docs = [
        f
        for f in file_data["all_files"]
        if os.path.basename(f) in DOCUMENTATION_FILES
        or "docs/" in f.replace(repo_path + "/", "")
        or "doc/" in f.replace(repo_path + "/", "")
    ]
...
def _detect_test_frameworks(
    all_files: list[str], build_systems: list[str]
) -> list[str]:
